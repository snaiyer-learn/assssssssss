...
class GoBoard(object):
    ...
    def end_of_game(self) -> bool:
        return self.win_with_five or self.captures[BLACK] >= 10 or self.captures[WHITE] >= 10 or not self.get_empty_points().any()
    ...
    def play_move(self, point: GO_POINT, color: GO_COLOR) -> bool:
        ...
        # removes suicidces and ko for ninuki 
        opp_color = opponent(color)
        self.board[point] = color

        # self._detect_and_process_capture(point)
        neighbors = [1, -1, self.NS, -self.NS, self.NS + 1, self.NS - 1, -self.NS + 1, -self.NS - 1]
        opp_color = opponent(self.get_color(point))
        for nb in neighbors:
            if self.board[point + nb] == BORDER:
                continue
            if self.board[point + nb] == opp_color:
                if self.board[point + nb + nb] == BORDER:
                    continue
                if self.board[point + nb + nb] == opp_color:
                    if self.board[point + nb + nb + nb] == color:
                        self.board[point + nb] = EMPTY
                        self.board[point + nb + nb] = EMPTY
                        self.captures[opp_color] += 2 
        
        # check for five in a row
        for nb in neighbors:
            if self.board[point + nb] == color:
                if self.board[point + nb + nb] == color:
                    if self.board[point + nb + nb + nb] == color:
                        if self.board[point + nb + nb + nb + nb] == color:
                            self.win_with_five = True
                            return True
    ...
    def _allneighbors(self, point: GO_POINT) -> List:
        """ List of all four neighbors of the point """
        return [point - 1, point + 1, point - self.NS, point + self.NS, point - self.NS - 1, point - self.NS + 1, point + self.NS - 1, point + self.NS + 1]
