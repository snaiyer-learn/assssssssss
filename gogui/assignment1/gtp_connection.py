...
class GtpConnection:
    ...
    def gogui_rules_final_result_cmd(self, args: List[str]) -> None:
        if self.board.end_of_game():
            ...
            elif self.board.captures[2] >= 10:
                self.respond("black")
                
            elif self.board.captures[1] >= 10:
                self.respond("white")
    ...
    def gogui_rules_legal_moves_cmd(self, args: List[str]) -> None:
        ...
        legal_moves = []
        for move in moves:
            legal_moves.append(format_point(point_to_coord(move, self.board.size)))
        legal_moves = " ".join(sorted(legal_moves))
    ...
    def genmove_cmd(self, args: List[str]) -> None:
        ...
        if self.board.end_of_game():
            if self.board.get_empty_points().any():
                self.respond("resign")
                return
            else:
                self.respond("pass")
    ...
    def gogui_rules_captured_count_cmd(self, args: List[str]) -> None:
        ...
        self.respond(str(black) +" "+ str(white))
